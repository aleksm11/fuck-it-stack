...
describe('initRouter (DOM)', () => {
  ...
  it('dispatches fis:route event', async () => {
    // compiledRoutes may already be set from prior tests; set up fresh
    initRouter([{ route: '/', file: '/pages/index.html' }]);
    await new Promise(r => setTimeout(r, 10)); // let initial navigate finish
    const outlet = document.getElementById('app');
    const handler = vi.fn();
    outlet.addEventListener('fis:route', handler);
    // Navigate explicitly (navigate is async, we can await it)
    await navigate('/');
    expect(handler).toHaveBeenCalled();
    expect(handler.mock.calls[0][0].detail.path).toBe('/');
  });
});
